...
export default function FounderSettings() {
  ...
  const saveProfile = async (e) => {
    ...
    try {
      ...
      setUser(updated);
      localStorage.setItem('worksyne_user', JSON.stringify(updated));
  ...
  return (
    <div style={{ maxWidth: 560 }}>
        ...
        {!editProfile ? (
          <div>
            {[
              ['✉️', 'Email',       user?.email],
              ['📞', 'Phone',       user?.phone || '—'],
              ['🏷️', 'Designation', user?.designation || '—'],
            ].map(([icon, label, val]) => (
            ...
            <button
              onClick={() => { setEditProfile(true); setProfileForm({ name: user?.name || '', phone: user?.phone || '', designation: user?.designation || '' }); }}
              className="btn-primary"
              style={{ marginTop: 8, width: '100%', background: 'rgba(245,230,66,0.1)', color: '#f5e642', border: '1px solid rgba(245,230,66,0.3)' }}
            >
              ✏️ Edit Profile
            </button>
          </div>
          ...
          <form onSubmit={saveProfile}>
            ...
            <input value={profileForm.phone} onChange={e => setProfileForm(p => ({ ...p, phone: e.target.value }))} className="input" placeholder="Phone number" style={{ marginBottom: 12 }} />

            <label style={{ display: 'block', fontSize: 11, color: '#9ca3af', marginBottom: 5 }}>Designation</label>
            <input value={profileForm.designation} onChange={e => setProfileForm(p => ({ ...p, designation: e.target.value }))} className="input" placeholder="e.g. Co-Founder" style={{ marginBottom: 18 }} />
      ...
      {/* ── Password Card ── */}
      <div className="card" style={{ marginBottom: 20 }}>
        <h2 style={{ fontSize: 15, fontWeight: 700, color: '#fff', marginBottom: 4, display: 'flex', alignItems: 'center', gap: 6 }}>
          🔐 Change Password
        </h2>
